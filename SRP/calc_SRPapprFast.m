...
%         
%         % samples
%         xi_mm_samp = real(exp(1j*nT{p}*omega.')*psi); % (10); nDT{p}*omega.' is N x K matrix, psi is K x 1 vector
%         
%         % interpolate for different values of N_aux
% 
%         xi_mm_int(:) = sinc(Delta_t_i(:,p)/T - n(1:end))*xi_mm_samp(1:end); % (14); Delta_t(:,p)/T - n is J x N matrix, xi_mm_samp is N x 1 vector
% 
%         SRP = SRP + 2*xi_mm_int; % (11)
%         
%     end
